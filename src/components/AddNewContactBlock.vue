<template>
  <div class="inputs_wrapper">
    ...
    <my-button-big @click="addContactEmit" class="add_contact_btn"> add contact </my-button-big>
  </div>

  <div class="error_message" :class="{ error_message_night: nightMode }">
    {{ errorMessage }}
  </div>
</template>
...
export default {
  emits: {
    addContactEmit: function (inputName, inputPhoneNumber, inputCity) {
      if (!isFinite(+inputName) && !isFinite(+inputCity)) {
  ...
  methods: {
    ...
    addContactEmit() {
      ...
      this.$emit(
        'addContactEmit',
        this.inputName[0].toUpperCase() + this.inputName.slice(1),
        this.inputPhoneNumber,
        this.inputCity[0].toUpperCase() + this.inputCity.slice(1)
      )
      // очищаем инпуты
      this.inputName = ''
      this.inputPhoneNumber = ''
