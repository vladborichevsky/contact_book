<template>
  ...
  >
    ...
    <div class="edit_inputs_wrapper">
      ...
      <modal-edit-contact-elem :notation="notations.city" v-model.trim="editCity" tabindex="3" />
    </div>

    <div class="edit_btns_wrapper">
      <my-button tabindex="4" @click="saveChanges" class="save_btn"> save </my-button>
...
export default {
  ...
  emits: {
    saveChanges: function (editName, editCity) {
      if (!isFinite(+editName) || !isFinite(+editCity)) {
        // проверяем, чтобы в инпуты с именем и городом не были введены цифры
        return true
      } else {
  ...
  data() {
    return {
      ...
      editCity: '',
      phoneNumberMask: {
        // этот объект необходлим для маски ввода номера телефона. Здесь же как раз можно менять маску.
  ...
  methods: {
    saveChanges() {
      this.$emit(
        'saveChanges',
        this.editName[0].toUpperCase() + this.editName.slice(1),
        this.editPhoneNumber,
        this.editCity[0].toUpperCase() + this.editCity.slice(1),
        this.showEditWindowObj.index
      )
