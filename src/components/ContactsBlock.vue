<template>
  ...
  <contacts-messages :filtredContactsArr="filtredContactsArr" :contactListArr="contactListArr" />
...
export default {
  ...
  props: {
    contactListArr: {
      // основной массив с контактами
      type: Array,
      required: true,
    },
  },

  emits: ['showDeleteModal', 'showInfoModal', 'showEditModal'],
  ...
  methods: {
    ...
    filterContactListArrWithEmptyFilterVar() {
      ...
      this.filtredContactsArr = JSON.parse(JSON.stringify(this.contactListArr)).sort((a, b) =>
    ...
    filterContactListArr() {
      this.filtredContactsArr = this.contactListArr
        .filter((item) =>
          item[this.sortBy].includes(this.filter[0].toUpperCase() + this.filter.slice(1))
        )
  ...
  watch: {
    ...
    filter() {
      if (this.filter.length == 0) {
        ...
        window.history.pushState(
          null,
          document.title,
          `${window.location.pathname}?sort_by=${this.sortBy}&page=${this.activePage}`
        )
      } else {
        // функция, которую мы используем, если в переменной filter что-то введено
        this.filterContactListArr()
        ...
        window.history.pushState(
          null,
          document.title,
          `${window.location.pathname}?sort_by=${this.sortBy}&filter=${this.filter}&page=${this.activePage}`
        )
      }
    },
    ...
    activePage() {
      // отслеживаем изменения переменной activePage, чтоб добавить данные о текущем состоянии активной страницы и информации в фильтре (переменная filter) в объект window.history (в виде строки), используя метод pushState()
      if (this.filter.length == 0) {
    ...
    if (windowData.page) {
      ...
      if (windowData.page == '0' && !windowData.filter) {
        ...
        window.history.pushState(
          null,
          document.title,
          `${window.location.pathname}?page=${this.activePage}`
        )
      } else {
