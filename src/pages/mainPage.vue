<template>
  ...
  <div class="page_wrapper" :class="{ night_mode_class: nightMode }">
    <div class="container">
      ...
      <add-new-contact-block :sameUserError="sameUserError" @addContactEmit="addContactFunc" />
...
export default {
  ...
  setup() {
    const messagesKit = {
      // объект с текстами для сообщений
  ...
  watch: {
    contactListArr: {
      // Все изменения (добавление/удаление/изменение контакта) в нашем главном массиве объектов с контактами сразу же записываем в localStorage
      handler() {
